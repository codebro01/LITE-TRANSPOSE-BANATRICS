...
export const vehicleDetailsTable = pgTable(
  'vehicle_details',
  {
    ...
    vehiclePhotos: jsonb('vehicle_photos')
      .$type<{
        secure_url: string;
        public_id: string;
      }>()
  ...
  (table) => ({
    idx_vehicleDetails_userId: index('idx_vehicleDetails_userId').on(table.userId),
    idx_vehicleDetails_userId_id: index('idx_vehicleDetails_userId_id').on(table.userId, table.id),
  }),
);
